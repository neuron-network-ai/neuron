...
w("installer — each node downloads its own slice from the upstream repository. But *serving*")
w("weights is distribution to end users, so a restricted licence binds the whole network")
w("rather than one machine.")
w("")
w("**This is enforced in code, not by convention.** `coordinator/model_registry.py` requires")
w("every model to declare a licence and refuses any that is not in `PERMITTED_LICENSES`.")
w("An absent or unrecognised licence is refused exactly like a restricted one, because")
w("\"we did not check\" must not look like \"we checked and it is fine\". Before that gate,")
w("`NEURON_EXTRA_MODELS` was an environment variable — restricted weights could enter the")
w("catalog with no code change and no record. Adding one now takes a diff someone has to")
w("justify. Covered by `coordinator/test_model_license_gate.py`.")
w("")
w("- **Qwen2.5 at 0.5B / 1.5B / 7B / 14B / 32B** — Apache 2.0. This is what NEURON serves.")
w("- **Qwen2.5 at 3B and 72B** — the separate Qwen and Qwen-Research licences, *not* Apache")
w("  2.0. Reaching for a bigger Qwen looks like a size change and is actually a licence")
w("  change; the gate refuses these.")
w("- **Llama-family models** — the Llama Community Licence, which is *not* an open-source")
w("  licence: acceptable-use policy, a 700M monthly-active-user ceiling, a required \"Built")
w("  with Llama\" attribution, and a naming rule for derivatives. Refused by the gate.")
w("  Benchmarking one locally is fine; serving one to users is a decision with obligations.")
w("")
