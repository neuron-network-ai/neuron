...
machines are slower) and the driver carries the output head — shown in the full per-node utilisation breakdown.
...
| `node_c` | middle layers (relay) | Linux laptop (Ryzen) | 4 cores | 11 GB | Ubuntu 24.04 |
| `node_b` | last layers + norm | Linux desktop | 6 cores | 15 GB | Ubuntu |
| coordinator | registry / routing / ledger | Linux desktop (always-on) | — | — | Ubuntu |
...
**Session 7 complete** — the coordinator is deployed to an always-on host (the desktop,
