...
rsync -az --info=stats1 $DRY -e "ssh -i $KEY -o StrictHostKeyChecking=accept-new" \
  --exclude='__pycache__' --exclude='*.pyc' --exclude='neuron.db*' --exclude='test_*.py' \
  "$here/coordinator/" "$HOST:$REMOTE/coordinator/"
rsync -az $DRY -e "ssh -i $KEY -o StrictHostKeyChecking=accept-new" \
  "$here/relay_auth.py" "$here/common.py" "$HOST:$REMOTE/"

if [ -n "$DRY" ]; then say "dry run complete — nothing changed"; exit 0; fi
