...
_DEFAULT_TIERS = [
    ...
    # min_nodes 2 the live 3-node network clears the promote margin, and the founder's
    # 16-core Windows PC (`agent-optinovate-67e4eb`, the big one) makes it placeable even at
    # fp32 — so the network would migrate itself onto a 4B model on a health sweep, which is
    # not what "run a model your machine can't run" is supposed to mean. Reach it with
    # `pinned_model_id`, deliberately, on the roster you meant.
    # (That machine reads 68 GB on the roster: `agent.py` reports decimal GB and 64 GiB is
    # 68.7 of them. 64 is the hardware; 68 is the unit. Neither is a typo.)
    #
